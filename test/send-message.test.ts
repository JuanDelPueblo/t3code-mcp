...
function snapshotFixture(): T3ThreadDetailSnapshot {
  return {
    snapshotSequence: 77,
    thread: {
      id: "thread-existing",
...
function captureTools(client: unknown): Map<string, ToolHandler> {
  ...
  registerTools(fakeServer as never, client as never);
  return tools;
}
...
describe("t3_send_message", () => {
  ...
  it("targets the supplied thread and dispatches thread.turn.start without creates", async () => {
    ...
    expect(result.content[0].text.toLowerCase()).toContain("dispatched");
  });

  it("reuses model/runtime config and leaves branch/worktree placement unchanged", async () => {
    ...
    const command = dispatched[0] as Record<string, unknown>;
    expect(command.modelSelection).toEqual(snapshot.thread.modelSelection);
    expect(command.runtimeMode).toBe("auto-accept-edits");
    expect(command.interactionMode).toBe("plan");
    // Placement is preserved by not sending any bootstrap/placement fields.
  ...
  it("waitMs=0 returns without subscribing for events", async () => {
    const { client } = fakeClient();
    const subscribe = (client as unknown as { subscribeThread: ReturnType<typeof vi.fn> }).subscribeThread;
    const tools = captureTools(client);
    const result = await tools.get("t3_send_message")!({
      threadId: "thread-existing",
      prompt: "continue",
  ...
  it("defaults to waitMs=0 when omitted", async () => {
    const { client } = fakeClient();
    const subscribe = (client as unknown as { subscribeThread: ReturnType<typeof vi.fn> }).subscribeThread;
    const tools = captureTools(client);
    await tools.get("t3_send_message")!({ threadId: "thread-existing", prompt: "hi" });
    expect(subscribe).not.toHaveBeenCalled();
  });

  it("nonzero waitMs collects the same response-event format used elsewhere", async () => {
